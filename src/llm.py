...
class GeminiBackend:
    ...
    def resolve_model(self) -> str:
        """Confirm the configured model exists, else fall back to a Flash model.

        Google renames models and revises the free tier without notice, so a
        hardcoded id is a liability the day before a deadline. If the configured
        one is unavailable, pick a Flash model the account can actually call.
        """
        try:
            available = [
        ...
        flash = [m for m in available if "flash" in m and "lite" not in m and "vision" not in m]
        fallback = flash or [m for m in available if "flash" in m] or available
